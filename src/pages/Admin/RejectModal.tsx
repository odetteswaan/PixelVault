...
const RejectModal = (props: { open: boolean; handleClose: () => void,userId:string|number }) => {
...
axios.patch(`${baseUrl}${approveUser(id)}`,{"status":"rejected"},{
  headers:{
    token:token
  }
}).then(()=>{
  ...
  return (
    ...
    >
      {loading?<Loader/>:<ModalContainer>
        ...
        <StyledHeading >Reject This User?</StyledHeading>
        ...
        <StyledBox>
         <Button variant="contained" color="error" onClick={()=>ApproveUser(props.userId)}>Reject</Button>
         <Button variant='contained' color="success" onClick={props.handleClose}>close</Button>
