...
import { baseUrl,assetDetail ,assetHistory} from 'src/config';
const AssetDetail = () => {
  const[loading,setLoading]=useState(true)
  const [assetData,setData]=useState<assestDetailType|null>(null);
  const[allocationLogs,setAllocation]=useState<AssetAssignment[]|null>(null)
  ...
  useEffect(()=>{
    axios.get(`${baseUrl}${assetHistory(productId)}`,{headers:{token:token}}).then(res=>{
      console.log(res.data)
         setAllocation(res.data)
    }).catch()
axios.get(`${baseUrl}${assetDetail(productId)}`,{
  headers:{
    token:token
  }
}).then(data=>{
              ...
              <Grid size={12}>
                <Box className="detailsContainer assetType">
                  <Box className="AssetContainer">
                    <img src={assetData?.images[0]} alt="AssetType" className="assetImage" />
                    <Box className="detailContainer">
                      ...
                      <Box className="btnContainer">
                        ...
                        <CustomBtn className="deallocateBtn" onClick={handleInvoice}>
                          View Invoice
                        </CustomBtn>
                      </Box>
                    </Box>
                  ...
                  <Grid container className="fullWidth" spacing={1.4}>
                    ...
                    {allocationLogs?.map((item, index) => (
                      <React.Fragment key={index}>
                        ...
                        <Grid size={3}>
                          {' '}
                          <Button className={item.status==="allocated"?"allocateBtn":"returnBtn"} variant="contained">
                            {item.status}
                          </Button>
                        </Grid>
                        <Grid size={3} className="none">
                    ...
                    <Grid size={12}>
                      <Box className="moreOptionContainer">
                        <Typography
                          ...
                          onClick={() => Navigate('/admin/employee-asset-allocation')}
                        >
          ...
          <Grid size={{ lg: 6, md: 12 }}>
            <Grid container className="fullWidth" rowSpacing={3}>
                  ...
                  <Grid container spacing={3}>
                    ...
                    <Grid size={12}>
                      <hr className="horizontalRule" />
                        ...
                        <Grid size={{xs:6,sm:6,lg:4,xl:4}} className="large">
                          <Box>
              ...
              <Grid size={12}>
                ...
                <Box className="detailsContainer profileFlex">
                  <Box className="profileContainer">
                    ...
                    <Box className="btnContainer">
                      ...
                      <CustomBtn className="deallocateBtn">
                        Dealloacte
                      </CustomBtn>
                    </Box>
                  </Box>
                ...
                :
                <CustomBtn className='deallocateBtn'>Allocate Asset</CustomBtn>
               }
              </Grid>
            </Grid>
          </Grid>
